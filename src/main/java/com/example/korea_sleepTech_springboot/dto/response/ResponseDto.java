...
@Data // getter, setter, toString을 동시 생성
@AllArgsConstructor(staticName = "set") // 모든 필드 값을 매개변수로 받는 생성자 + staticName 속성(속성값 "set")
// : set()이라는 이름의 정적 메서드를 자동으로 생성
public class ResponseDto<D> {
    private boolean result; // 요청의 성공 여부 (true || false)
    private String message; // 요청 처리에 대한 설명 메시지
    private D data; // 실제 응답 데이터 (EX, 유지 정보, 게시글 목록 등) - 제네릭으로 어떤 타입이든 가능
