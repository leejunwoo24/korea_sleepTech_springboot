...
public class D_Post {
    ...
    *   - comments 리스트에 Comment를 추가, 해당 Comment의 post 필드에 현제 Post 객체를 선언
    *   >> 해당 설정을 하지 않으면 JPA의 영속성 컨텍스트가 양방향 관계를 완전하게 이해하지 못 함
    *
    *   cf) 영속성 컨텍스트
    *       : JPA에서 엔티티 객체를 연구 저장하는 환경을 의미
    *       - 엔티티 메니저(EntityManager)에 의해 관리, save(), remove(), find()와 같은 작업을 수행
    ...
    *       - Post가 여러 개의 Comment를 가짐(@OneToMany) - Comment는 하나의 Post에 속함(@ManyToOne)
    *
    ...
    * */
    ...
    public void addComment(D_Comment comment){
        ...
        comment.setPost(this); // Commnet 객체에 현재의 Post를 설정
    }
    ...
    *       >> 해당 과정에서 orphanRemoval = true(고아 객체 선언)로 인하여 실제 DB에서도 동시 삭제
    * */
