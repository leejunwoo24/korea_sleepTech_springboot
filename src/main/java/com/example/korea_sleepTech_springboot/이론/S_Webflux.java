...
*       - DB, 메일, 파일 IO 등 시간 오래 걸리는 작업을 별도 쓰레드에서 수행 가능
*
* 4. WebFlux 도입 시 주의점
*       - JpaRepositroy 블로킹
*           : JPA를 사용한다면 완전한 리액티브가 아님! (R2DBC로 변경해야 완전한 논블로킹)
*       - 모든 리턴 타입이 Mono 또는 Flux
*           : Spring MVC처럼 단순히 String 또는 ResponseEntity를 리턴하면 안 됨!
*       - 스케줄러 선택 중요
...
* [3] JWT 토큰 생성 + 이메일 발송 (BoundedElasitc 스케줄러)
* [4] 응답: 인증 이메일 전송 완료
* */
