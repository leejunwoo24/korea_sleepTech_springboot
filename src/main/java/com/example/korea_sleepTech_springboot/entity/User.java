...
// : 기본 생성자 생성
//      - AccessLevel.PROTECTED를 통해 외부에서 new 키워드로 객체 생성 불가 설정
...
public class User implements UserDetails {
    ...
    @Column(name = "id", updatable = false) // DB 컬럼명 설정, updatable = flase로 수정 불가 설정
    private Long Id;
    ...
    // cf) LAZY VS EAGER
    // cf) Lazy(지연 로딩): 해당 데이터와 해당 필드가 같이 사용되지 않는 경우
    // EAGER(즉시 로딩): 해당 데이터와 해당 필드가 동시에 사용되는 경우
    @JoinTable(
