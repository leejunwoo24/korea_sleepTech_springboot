...
public class WebSecurityConfig {
    ...
    * : UsernamePasswordAuthenticationToken 이전에 동작
    ...
    * */
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        return http
                .csrf(AbstractHttpConfigurer::disable) // CSRF(Cross-Site Request Forgery) 공격 방지 설정 비활성화(JWT 보안 토큰 기반 인증 덕분에 필요 없음)
                .cors(withDefaults()) // 다른 도메인간 요청을 허용 여부를 결정하는 메서드 CORS 설정을 활성화
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers( // : 특정 요청과 일치하는 url에 대한 엑세스, 특정 HTTP 요청 경로(URL 패턴) 에 대해서 접근 제어 규칙을 설정하는 메서드
                                // 특정 경로에 대한 엑세스 설정
                                // : 인증 처리 없이 접근 가능
                                new AntPathRequestMatcher("/api/v1/auth/**"), //
                                new AntPathRequestMatcher("/api/v1/admin/**") //경로 패턴을 이용해 요청 URL을 매칭하는 데 사용되는 도구
    ...
    * */
    ...
    public AuthenticationManager authenticationManager(BCryptPasswordEncoder bCryptPasswordEncoder) throws Exception {
        ...
        // ProviderManaer를 반환: DaoAuthenticationProvider를 사용ㅎ여 인증 처리
    ...
    * - 단바향 해시함수를 사용하여 비밀번호 암호화 함 (복호화 할 수 X)
    *
    * cf) 복호화: 암호를 복구하다
    * */
    @Bean
