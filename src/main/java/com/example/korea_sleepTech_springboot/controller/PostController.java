...
public class PostController {
    ...
    @PostMapping
    // @Valid
    // : DTO 객체에 대한 검증을 수행하는 에너테이션
    // : 사용자가 클라이너트로부터 전달한 데이터가 미리 정의된 규칙에 맞는지 확인
    ...
    public ResponseEntity<ResponseDto<List<PostWithCommentCountResponseDto>>> getTop5PostsByComments(){
        ResponseDto<List<PostWithCommentCountResponseDto>> response = postService.getTop5PostByComment();
        return ResponseEntity.status(HttpStatus.OK).body(response);
    }
    ...
    //      >> author가 작성한 게시글 중에서 댓글이이 3개 이상인 게시글을 조회
}
