...
//        }
        ...
        // cf 게시물 삭제 이전에 모든 댓글에 대해 관계를 해제
        // 인스턴스 메서드 참조
        // 기본) 인스턴스명.메서드명()
    ...
    @Override
    @Transactional(readOnly = true)
    public ResponseDto<List<PostWithCommentCountResponseDto>> getTop5PostByComment() {
        List<PostWithCommentCountResponseDto> responseDtos = null;

        List<Object[]> posts = postRepository.findTop5ByOrderByCommentSizeDesc();

        responseDtos = posts.stream()
                .map(row -> PostWithCommentCountResponseDto.builder()
                        // JPA 데이터 반환이 Object[] 타입
                        // : 내부의 데이터는 Object로 명시 >> 각 타이으로 명시적 형 변환 (강제 형 변환)
