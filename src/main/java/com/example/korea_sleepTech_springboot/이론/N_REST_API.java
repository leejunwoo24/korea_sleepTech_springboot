...
public class N_REST_API {
    ...
    *  - transfer: 옮기다(이전하다)
    *
    * >> 자원을 이름(자원의 표현)으로 표현하고, HTTP 메서드를 통해 자원을 처리하는 아키텍쳐 스타일
    ...
    *  cf) REST API 학습 전 선행 키워드
     * 1) HTTP 프로토콜: 웹에서 클라이너트-서버 간 데이터를 주고받기 위한 통신 규약 (GET, POST, PUT, DELETE 등)
    ...
    *           로그인한 사용자 정보 수정: api/v1/users/me/password (PUT)
    ...
    *
    * - /auth
    *       : 인증 관련 처리(로그인, 로그아웃, 회원가입 등)
    *       EX) api/v1/auth/login
    *           api/v1/auth/logout
    *           api/v1/auth/signup-email?email=test@example.com
    *
    * - /dashboard
    ...
    * - 특정 배송 방법의 주문 목록: GET /api/v1/deliverys/{deliveryId}/orders
    * */
}
