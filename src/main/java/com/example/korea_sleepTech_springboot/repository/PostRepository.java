...
public interface PostRepository extends JpaRepository<D_Post, Long> {
    ...
    // cf) 추가 조건
    //      - findByAuthorAndTitle(Stirng author, String title);
    //      - findByAuthorOrTitle(String author, Sring title);
    //      - findByAuthorOrderByCreatedAtDesc(String author);
    //          >> 작성자 이름으로 조회 + 최신순으로 정렬
    // List<D_Post> findByAuthor(String author);

    // cf) @Query 애너테이션
    //     : 직접 JPQL이나 SQL를 작성해서 쿼리를 실행시켜주는 애너테이션
    //     : 쿼리 메서드로 표헌하기 어려운 복잡한 조건을 처리할 때 사용 (EX. JOIN)
...
//    List<D_Post> findByAuthor(@Param("author") String author);
    ...
    @Query(value = "SELECT p FROM post WHERE author = :author", nativeQuery = true)
    List<D_Post> findByAuthor(@Param("author") String author);

    // === 2) 특정 키워드로 제목 검색 === //
    // List<D_Post> findByTitleContaining(String title);
    List<D_Post> findByTitleIgnoreCaseContaining(String title); // 대소문자 구분 업시 검색
    ...
    @Query(value = """
                ...
                FROM post p 
                    ...
                    ORDER BY COUNT(c.id) DESC
                     LIMIT 5
                     """, nativeQuery = true)
    List<Object[]> findTop5ByOrderByCommentSizeDesc(); // Object 클래스는 모든 클래스의 최상위 클래스
 }
