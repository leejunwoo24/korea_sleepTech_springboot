...
@Entity // DB 테이블과 1:1매핑, JPA 엔티티임을 선언(반드시 PK 기본키가 필요)
...
//        }
//)
// >> name과 email 둘 다 같은 데이터가 동시에 있으면 안 된다. (EX. (a, b) >> (c, b)가능, (a, e) 가능 (a, b)는 불가능)
@Getter
@Setter
@NoArgsConstructor
public class B_Student {

    @Id // 해당 필드가 테이블의 PK 기본키임을 명시
    @GeneratedValue(strategy = GenerationType.IDENTITY) // 기본키 값을 자동 생성 - DB의 AUTO_INCREMENT 사용
    private Long id;


    // final 또는 @NonNull 사용 시 @RequiredConstructor에서 매게변수 사용
    private String name;
